...
% meloperation/8 - defines the meloperation predicate.
meloperation(OpMax, OpMin, [M, X], [N, X], [R, X]) :-
    u(X),  % Ensure valid element
    sign(M, SM),
    sign(N, SN),
% select_operation/7 - defines the select operation predicate.
...
% update_vector/4 - defines the update vector predicate.
...
% mset_normalize/2 - defines the mset normalize predicate.
...
% mset_intersect/3 - defines the mset intersect predicate.
