...
% f2 = "flip two coins"
% q_pair/2 - defines the q pair predicate.
...
% open_dot_file/3 - defines the open dot file predicate.
open_dot_file(Filename, Fd, Label) :- open(Filename, write, Fd), 
	write(Fd, 'digraph G {\n'),
	write(Fd,'label=\"'),write(Fd,Label), write(Fd,'\";\n'),
	write(Fd, 'layout=circo;\n'),
	write(Fd, 'rankdir=LR;\n'),
% write/2 - defines the write predicate.
	write(Fd, 'orientation=portrait;\n').
%	write(Fd, 'size =\"4,6\";\n').

% close_dot_file/1 - defines the close dot file predicate.
close_dot_file(Fd) :- write(Fd,'}\n'), close(Fd).

% make_vertex_uuid_pair/3 - defines the make vertex uuid pair predicate.
make_vertex_uuid_pair(V,[V,UUID]) :- uuid(UUID,[format(integer)]).
% make_vertex_uuid_pairs/2 - defines the make vertex uuid pairs predicate.
make_vertex_uuid_pairs(Vs,Pairs) :- maplist(make_vertex_uuid_pair,Vs,Pairs).
% writeln/1 - defines the writeln predicate.
writeln(X) :- write(X), nl.
% writelist/1 - defines the writelist predicate.
writelist(X) :- maplist(writeln,X).

% Write ternary relations to dot file:
% Build ordered lists for domain and range:
% write_ternary_dot_pair/5 - defines the write ternary dot pair predicate.
...
% write_ternary_dot_pairs/2 - defines the write ternary dot pairs predicate.
...
% has_Msubset/3 - defines the has Msubset predicate.
has_Msubset(X,M,Sub):- has_Msubset(X,M,0,[],[],Sub).
% has_Msubset/6 - defines the has Msubset predicate.
...
% bin2tern/6 - defines the bin2tern predicate.
bin2tern([[X,Y],Z],[X,Y,Z]).
% tern2bin/6 - defines the tern2bin predicate.
tern2bin([X,Y,Z],[[X,Y],Z]).

% write_latex_triple/3 - defines the write latex triple predicate.
write_latex_triple([X,Y,Z]) :- write('('),write(X),write(','),write(Y),write(','), write(Z),write(')').

% write_latex_triple_set/2 - defines the write latex triple set predicate.
...
% gen_dot_filename/1 - defines the gen dot filename predicate.
gen_dot_filename(Filename) :- gen_filename('.dot',Filename).
% gen_dot_filename/2 - defines the gen dot filename predicate.
gen_dot_filename(Path,Filename) :- gen_filename(Path,'.dot',Filename).

% gen_filename/2 - defines the gen filename predicate.
...
% gen_filename/3 - defines the gen filename predicate.
...
% gen_rand_graph/3 - defines the gen rand graph predicate.
...
% gen_rand_pair_graph/3 - defines the gen rand pair graph predicate.
...
% make_dot_file/1 - defines the make dot file predicate.
...
% complete_graph/2 - defines the complete graph predicate.
...
% write_dot_pair/3 - defines the write dot pair predicate.
...
% write_dot_pairs/2 - defines the write dot pairs predicate.
write_dot_pairs(Pairs,Fd) :- forall(member([X,Y],Pairs),write_dot_pair([X,Y],Fd)).
% list_to_atom/3 - defines the list to atom predicate.
list_to_atom([X,Y],Atom) :- atomic_list_concat(['[',X,',',Y,']'],Atom).
% edge_to_dot/3 - defines the edge to dot predicate.
edge_to_dot(X-Y,VPairs,DotEdge) :- member([X,Xuid],VPairs), member([Y,Yuid],VPairs), 
% atomic_list_concat/5 - defines the atomic list concat predicate.
									atomic_list_concat([Xuid,'->',Yuid,';\n'],DotEdge).

% ugraph_to_dot/2 - defines the ugraph to dot predicate.
...
% ugraph_to_dot/3 - defines the ugraph to dot predicate.
...
% ugraph_to_dot/4 - defines the ugraph to dot predicate.
...
% vertex_pair_to_label_string/3 - defines the vertex pair to label string predicate.
...
% vertex_pair_to_label_string/4 - defines the vertex pair to label string predicate.
...
% union_collection/2 - defines the union collection predicate.
union_collection(Collection,Union) :- union_collection(Collection,[],Union).
% union_collection/3 - defines the union collection predicate.
...
% disjoint/2 - defines the disjoint predicate.
disjoint(A,B) :- intersection(A,B,[]).
% disjoint_from_collection/2 - defines the disjoint from collection predicate.
disjoint_from_collection(A,Collection) :- subtract(Collection,[A],C),
			forall(member(X,C),disjoint(X,A)).
% disjoint_collection/1 - defines the disjoint collection predicate.
...
% partition/2 - defines the partition predicate.
partition(C,A) :- disjoint_collection(C), union_collection(C,B), set_equal(A,B).

% binop_range_to_ugraph_component/3 - defines the binop range to ugraph component predicate.
binop_range_to_ugraph_component(BinOp,Z,U) :- range(BinOp,Rng), member(Z,Rng),
									findall(Pairs,(member([Pairs,Z],BinOp)),Vs),
% complete_graph/2 - defines the complete graph predicate.
									complete_graph(Vs,U).
% findall_components/2 - defines the findall components predicate.
...
% shell/1 - defines the shell predicate.
    shell(Command).
