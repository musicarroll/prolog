...
% set_equal/2 - defines the set equal predicate.
...
% is_bijection/3 - defines the is bijection predicate.
...
% has_subset/2 - defines the has subset predicate.
has_subset(_, []).
% has_subset/2 - defines the has subset predicate.
...
% is_subset/2 - defines the is subset predicate.
...
% powerset/2 - defines the powerset predicate.
...
% index_to_subset/3 - defines the index to subset predicate.
index_to_subset(Index, Set, Subset) :-
    binary_rep(Index, Set, [], RevSubset),
% reverse/2 - defines the reverse predicate.
    reverse(RevSubset, Subset).

% Generate a subset from a binary representation
% binary_rep/4 - defines the binary rep predicate.
...
% has_subset_of_size/3 - defines the has subset of size predicate.
...
% collection_of_nonempty_sets/2 - defines the collection of nonempty sets predicate.
collection_of_nonempty_sets(C,X) :- is_set(X), \+X=[], 
	powerset(X,P), !, 
	subtract(P,[[]],NP), 
	has_subset(NP,S), \+S=[], 
% sort/2 - defines the sort predicate.
	sort(S,C).

% Utility predicate that orders a set and generates its ordered Cartesian product.
% order_set_and_cartesian/3 - defines the order set and cartesian predicate.
order_set_and_cartesian(Set, OrderedSet, OrderedCartesian) :-
    sort(Set, OrderedSet),
% cartesian_product/3 - defines the cartesian product predicate.
...
% pair_with_all/3 - defines the pair with all predicate.
pair_with_all(_, [], []).
% pair_with_all/4 - defines the pair with all predicate.
pair_with_all(A, [B|Bs], [[A,B]|Paired]) :-
% pair_with_all/3 - defines the pair with all predicate.
    pair_with_all(A, Bs, Paired).

% Older, less efficient implementations:
% cartesian/3 - defines the cartesian predicate.
cartesian(A,B,Cart):-
             findall([X,Y],(member(X,A),member(Y,B)),L),
             list_to_set(L,Cart),!.
 
% cartesian/2 - defines the cartesian predicate.
cartesian(A,Cart):- cartesian_product(A,A,Cart).


% Recursive definition to compute cartesian product of three lists
% cartesian_product3/4 - defines the cartesian product3 predicate.
...
% pair_with_all_bc/4 - defines the pair with all bc predicate.
...
% pair_with_all_c/4 - defines the pair with all c predicate.
pair_with_all_c(_, _, [], []).
% pair_with_all_c/6 - defines the pair with all c predicate.
...
% cartesian3/4 - defines the cartesian3 predicate.
cartesian3(A, B, C, Cart) :-
% cartesian_product3/4 - defines the cartesian product3 predicate.
    cartesian_product3(A, B, C, Cart).

% cartesian3/2 - defines the cartesian3 predicate.
cartesian3(A,Cart) :- cartesian3(A,A,A,Cart).


% relation/3 - defines the relation predicate.
...
% domain/2 - defines the domain predicate.
domain(PL,Dom) :-
			 pair_list(PL),
...
% Ditto for the range:
% range/2 - defines the range predicate.
range(PL, Rng) :-
			 pair_list(PL),
...
% range_list/2 - defines the range list predicate.
range_list(PL, RngList):-
             pair_list(PL),
             findall(Y,member([_,Y],PL),RngList).
 
% image/3 - defines the image predicate.
image(PL,X,ImgX) :-
             pair_list(PL),
             domain(PL,Dom),
             member(X,Dom),
...
% func_image/2 - defines the func image predicate.
func_image(PL,ImgX) :-
             image(PL,_,ImgX),
% length/2 - defines the length predicate.
             length(ImgX,1).
 
% function/1 - defines the function predicate.
function(PL) :-
             pair_list(PL),
             forall(image(PL,_,ImgX),func_image(PL,ImgX)).
 
% gen_func/2 - defines the gen func predicate.
...
% gen_func/4 - defines the gen func predicate.
...
% choose_val/2 - defines the choose val predicate.
choose_val(A,V):-member(V,A).

% gen_choice_func/3 - defines the gen choice func predicate.
...
% eval/3 - defines the eval predicate.
eval(F,X,Y) :- nonvar(F), nonvar(X),
             function(F),
% member/3 - defines the member predicate.
             member([X,Y],F).
% eval_on_set/3 - defines the eval on set predicate.
eval_on_set(F,S1,S2) :-
    findall(Y,(member(X,S1),eval(F,X,Y)),L), list_to_set(L,S2),!.

% eval_on_pair/5 - defines the eval on pair predicate.
eval_on_pair(F,[X,Y],[RX,RY]):- eval(F,X,RX), eval(F,Y,RY).

% make_pair_func/2 - defines the make pair func predicate.
...
% gen_bijection/2 - defines the gen bijection predicate.
...
% stabilizer_of/3 - defines the stabilizer of predicate.
...
% is_automorph_of/2 - defines the is automorph of predicate.
is_automorph_of(F,R):- forall(member([X,Y],R),
                        (eval_on_pair(F,[X,Y],[FX,FY]),member([FX,FY],R))).
% all_automorphs_of/3 - defines the all automorphs of predicate.
all_automorphs_of(R,SuperDom,Autos):- 
                findall(G,(gen_bijection(SuperDom,G),is_automorph_of(G,R)),L),
% list_to_set/2 - defines the list to set predicate.
                list_to_set(L,Autos).

% gen_automorph_rel/4 - defines the gen automorph rel predicate.
gen_automorph_rel([X0,Y0],F,R) :- gen_automorph_rel([X0,Y0],[],F,[],R).

% gen_automorph_rel/7 - defines the gen automorph rel predicate.
gen_automorph_rel([X0,Y0],[X0,Y0],_,R,R).
% gen_automorph_rel/6 - defines the gen automorph rel predicate.
...
% full_automorph_rel/3 - defines the full automorph rel predicate.
full_automorph_rel(SeedSet,F,Rel):-
    make_pair_func(F,PF),
% full_automorph_rel/5 - defines the full automorph rel predicate.
...
% subst_value/6 - defines the subst value predicate.
subst_value(Y,Z,[X,Y],[X,Z]).
% subst_value/5 - defines the subst value predicate.
subst_value(Z,[X,_],[X,Z]).
 
% subst_value_list/3 - defines the subst value list predicate.
subst_value_list([],List,List).
% subst_value_list/5 - defines the subst value list predicate.
subst_value_list([H|T],[[H1,_]|T1],[[H1,H]|T2]):-
% subst_value_list/3 - defines the subst value list predicate.
             subst_value_list(T,T1,T2).

% full_automorph_from_sset_vset/4 - defines the full automorph from sset vset predicate.
...
% check_uniqueness/2 - defines the check uniqueness predicate.
    check_uniqueness(Relation, Domain).

% Given a relation, return a list of all the X values
% get_domain/2 - defines the get domain predicate.
get_domain(Relation, Domain) :-
...
% Experiments:
% test1/4 - defines the test1 predicate.
...
% test_bijections/0 - defines the test bijections predicate.
test_bijections:- test_bijections([[a,b,c],R,Bs]),
% write_rel_bij/2 - defines the write rel bij predicate.
    write_rel_bij(R,Bs).

% test_bijections/3 - defines the test bijections predicate.
...
% all_bijections/3 - defines the all bijections predicate.
...
% test_all_bijections/0 - defines the test all bijections predicate.
